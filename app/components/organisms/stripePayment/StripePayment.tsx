"use client";
import { be_service, stripe_client_key } from "@/app/utils/Fixtures";
...
import { StripePaymentLoader } from "./StripePaymentLoader";
...
export const StripePayment: FC<{
  ...
  useEffect(() => {
    (async () => {
      ...
      const paymentIntent = await fetch(
        `${be_service}/stripe/create-payment-intent`,
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  }, [items, expectedCurrency]);

  if (!paymentIntent?.client_secret) return <StripePaymentLoader />;

  return (
    <div className="dark:text-white">
      <div className=" justify-between w-full flex text-xl mb-2">
        <svg
      ...
      <Elements
        options={{
          ...
          locale: getClientLang() as any,
        }}
        stripe={stripePromise}
      >
        <StripeForm
          stripePromise={stripePromise as any}
          client={client}
          t={t}
          validate={validate}
